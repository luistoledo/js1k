...
gl=function(){c.fillStyle=j;c.fillRect(0,0,W,H);j="#fde";t.forEach(ud);c.fillText("k:"+z+" h:"+p.b,0,9);1>p.b?v.stop():(v.frequency.setValueAtTime(f,ac.currentTime),f=0,mb&&.8<R()&&(t.push(no(p.x,p.y,fm(p,no(mx,my)),"Tan",0)),f=200),1>Date.now()%35&&e.push(no(g,g,g,"red",1)),e.forEach(ud),p.y+=.5*(k[83]|0-k[87]|0),p.x+=.5*(k[68]|0-k[65]|0),ud(p,0),requestAnimationFrame(gl))};ac=new AudioContext;v=ac.createOscillator();v.connect(ac.destination);v.start(0);gl();b.onkeydown=function(a){k[a.keyCode]=1};b.onkeyup=function(a){k[a.keyCode]=0};
